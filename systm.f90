MODULE SYSTM 
	...
	COMPLEX(cx),parameter :: ic=(0.0_8,1.0_8)
...
CONTAINS
	...
	SUBROUTINE get_parameters(sys)
		...
		real(rl), allocatable   		::  charge_op(:,:), h_pp(:,:)
		...
		if(nargs==0)then
			...
			do i=1,nargs
				...
				elseif(buffer=='-qb_ini')then 
					...
					call get_command_argument(i+1, buffer)
					read(buffer,*) sys%alpha_LF
				else if(buffer=='-cutoff_LF')then 
					call get_command_argument(i+1, buffer)
					read(buffer,*) sys%cutoff_LF
					sys%cutoff_LF = sys%cutoff_LF*twopi
				...
				else if(buffer=='-ncut')then 
					call get_command_argument(i+1, buffer)
					read(buffer,*) sys%ncut
				end if
		...
		do i=1,sys%nl
			do j=1,sys%nl
				read(101,'(f20.15)',advance='no') charge_op(i,j)
			end do
			read(101,*)
		end do
		...
		do i=1, sys%nl
			do j=1, sys%nl
				if ( abs( sys%nij(i,j) ) > sys%ncut*abs( sys%nij(1,2) )  ) then
					nij_abs_keep(i,j) = 1
				else
					sys%nij(i,j) = 0._8
				end if
	...
	SUBROUTINE update_sums( sys, st, sum_time )
		...
		real(8)		   			   				  :: t1, t2, tmp
		COMPLEX(cx), dimension(sys%nl, st%ncs)	  :: gyeff
		integer ::  m,n,i,j,ii,jj

		call cpu_time(t1)
		do i=1, size(st%y,1)
			st%y0(i,:) = matmul( st%y(i,:,:), sys%Omat(1,:) )
			!gyeff(i,:) = matmul( st%y(i,:,:), sys%g_qc*sys%Omat(1,:) )
			gyeff(i,:) = matmul( st%y(i,:,:), sys%gk(:) )
		end do
		...
		do m=1, size(st%y,2)
			...
			do i=1, size(st%y,1)
				st%ovm(i,m,i,m) = 1
				st%bigL(i,m,i,m) = sys%nij(i,i) * ( conjg(gyeff(i,m)) + gyeff(i,m) )**sys%expnt
			end do
			do i=1, size(st%y,1)
				do j=i+1, size(st%y,1)
					...
					st%bigL(i,m,j,m) = sys%nij(i,j) * (conjg(gyeff(i,m)) + gyeff(j,m))**sys%expnt
			...
			do n=m+1, size(st%y,2)
				...
				do i=1, size(st%y,1)
					...
					!st%ovm(i,n,i,m) = conjg(st%ovm(i,m,i,n))
					st%bigL(i,m,i,n) = sys%nij(i,i) * ( conjg(gyeff(i,m)) + gyeff(i,n) )**sys%expnt
				...
				do i=1, size(st%y,1)
					do j=i+1, size(st%y,1)
						...
						st%bigL(i,m,j,n) = sys%nij(i,j) * ( conjg(gyeff(i,m)) + gyeff(j,n) )**sys%expnt
						st%bigL(i,n,j,m) = sys%nij(i,j) * ( conjg(gyeff(i,n)) + gyeff(j,m) )**sys%expnt
	...
	FUNCTION parameterchar(sys)
		...
		character(len=20)      		:: ratio_char,nl_char,wc_char,ncsini_char,ncsmax_char,&
			...
			alpha_char,alpha_LF_char,cutoff_LF_char,nmodes_char,p0_char, qbini_char, cavini_char,  &
		...
		write( alpha_LF_char, '(f10.6)' ) sys%alpha_LF
		write( cutoff_LF_char, '(f10.3)' ) sys%cutoff_LF/twopi
		...
		parameterchar="nl"//trim(adjustl(nl_char))//&
			...
			"_"//trim(adjustl(alpha_LF_char))//&
			"_"//trim(adjustl(cutoff_LF_char))//&
